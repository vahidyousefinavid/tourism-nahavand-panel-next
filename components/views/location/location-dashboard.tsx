...
import { Location } from '@/types'; // فرض میکنم تایپ Location رو تعریف کردی مشابه انتیتی
import { AddLocationModal } from '@/components/views/location/add-location-modal';
import { EditLocationModal } from '@/components/views/location/edit-location-modal';
import { DeleteConfirmationModal } from '@/components/views/location/delete-confirmation-modal';
import { LocationDetailsModal } from '@/components/views/location/location-details-modal';
...
export function LocationDashboard() {
  ...
  useEffect(() => {
    const filtered = locations.filter(location =>
      ...
      location.organizer.toLowerCase().includes(searchTerm.toLowerCase()) ||
      location.price.toLowerCase().includes(searchTerm.toLowerCase()) ||
      location.latlang.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  const registeredLocations = locations.filter(l => l.registered && l.registered !== '0').length;
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">
                Manage your locations efficiently with our comprehensive dashboard
              </p>
            </div>
            <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <CardContent>
              ...
              <p className="text-xs text-gray-500">
                {totalLocations > 0 ? 'Locations in database' : 'No locations yet'}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-gray-900">{registeredLocations}</div>
              <p className="text-xs text-gray-500">
                {totalLocations > 0 ? `${Math.round((registeredLocations / totalLocations) * 100)}% registered` : 'N/A'}
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-gray-900">{recentLocations}</div>
              <p className="text-xs text-gray-500">
                Added in last month
              </p>
            </CardContent>
        ...
        <DeleteConfirmationModal
          ...
          name={selectedLocation ? selectedLocation?.title : ''}
        />
