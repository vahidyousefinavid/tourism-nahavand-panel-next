...
import { Textarea } from '@/components/ui/textarea'; // فرضا این کامپوننت موجوده
import { Dialog, DialogContent, DialogHeader, DialogTitle } from '@/components/ui/dialog';

interface LocationFormData {
  title: string;
  description: string;
  date: string; // رشته در قالب 'YYYY-MM-DD'
  location: string; // bigint به صورت رشته عددی
  latlang: string;
  price: string;
  capacity: string;
  registered: string;
...
export function AddLocationModal({ isOpen, onClose, onSubmit }: AddLocationModalProps) {
  ...
  const validateForm = () => {
    ...
    // اگر لازم باشه می‌توان اعتبارسنجی عددی روی price بذاریم
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => !isSubmitting && onClose()}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">افزودن مکان جدید</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-2 pt-4">
            ...
            >
              {isSubmitting ? 'در حال ارسال...' : 'افزودن مکان'}
            </Button>
          </div>
        </form>
      </DialogContent>
