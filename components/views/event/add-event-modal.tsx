...
import { CreateEventDto, EventTimeRangeDto } from '@/types';
...
export function AddEventModal({ isOpen, onClose, onSubmit }: AddEventModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const copy = { ...formData };
      delete copy.image;
      payload.append("dto", JSON.stringify(copy)); // ✅ حتما stringify
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => !isSubmitting && onClose()}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">افزودن رویداد جدید</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            >
              {isSubmitting ? 'در حال افزودن...' : 'افزودن رویداد'}
            </Button>
          </div>
        </form>
      </DialogContent>
