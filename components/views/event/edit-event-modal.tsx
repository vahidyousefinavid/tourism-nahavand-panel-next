...
import { Event, UpdateEventDto, EventTimeRangeDto } from '@/types';
...
interface EditEventModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (eventData: any) => void; // می‌تواند FormData یا JSON باشد
  event: Event | null;
}
...
export function EditEventModal({ isOpen, onClose, onSubmit, event }: EditEventModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const copy = { ...formData };
      delete copy.image;
      payload.append("dto", JSON.stringify(copy)); // ✅ تغییر اینجا

      await onSubmit(payload); // اینجا باید async/await باشه چون API call میشه
      setErrors({});
      onClose();
    } catch (err) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => !isSubmitting && onClose()}>
      <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">ویرایش رویداد</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                if (file) {
                  ...
                  reader.onloadend = () => {
                    handleChange('image', reader.result as string); // آپدیت state
                  };
                  reader.readAsDataURL(file); // فایل به DataURL تبدیل می‌شود
                }
              }}
            />
            {formData.image && (
                ...
                <img
                  src={
                    formData.image.startsWith('data:')
                      ? formData.image // فایل انتخاب‌شده از سیستم
                      : `${process.env.NEXT_PUBLIC_API_URL}${formData.image}` // مسیر بک
                  }
                  alt="پیش‌نمایش"
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            >
              {isSubmitting ? 'در حال بروزرسانی...' : 'بروزرسانی رویداد'}
            </Button>
          </div>
        </form>
      </DialogContent>
